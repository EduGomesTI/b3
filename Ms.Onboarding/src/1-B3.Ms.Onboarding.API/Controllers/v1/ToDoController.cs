...
namespace B3.DeafioTecnico.API.Controllers.v1
{
    ...
    public class ToDoController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(ToDoUpdateStatusResponse), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces(MediaTypeNames.Application.Json)]
        public async Task<IActionResult> UpdateToDoStatus([FromBody] ToDoUpdateStatusRequest request, CancellationToken cancellationToken)
        {
            if(!ModelState.IsValid)
            {
                return BadRequest();
